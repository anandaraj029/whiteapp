...
// Get the checklist type from the query parameter
$checklist_type = $_GET['checklist_type'] ?? ''; // Use null coalescing operator to avoid undefined error

// Output checklist type for debugging
echo "Checklist Type: " . htmlspecialchars($checklist_type) . "<br>";

// Fetch the data based on the checklist type
$sql = "SELECT * FROM checklist_information WHERE checklist_type = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param('s', $checklist_type);

if ($stmt->execute()) {
    $result = $stmt->get_result();

    // Output the record count for debugging
    ...
    if ($result->num_rows > 0) {
        ...
        echo "No data found for the given checklist type.";
        exit(); // Stop further execution if no data
    }
} else {
...
<body>
    ...
    <tr>
        ...
        <td style="width: 25%;"><?php echo htmlspecialchars($row['report_no']); ?></td>
        <th style="width: 25%;">INSPECTION DATE:</th>
        <td style="width: 25%;"><?php echo htmlspecialchars($row['inspection_date']); ?></td>
    </tr>
    <tr>
        <th>CLIENT’S NAME:</th>
